...
def on_connect(client, userdata, flags, rc):
    ...
    startSubscriptions(["Read","Write","MaskWrite","Disconnect","Run","Freq"])
...
def Freq_callback(client, userdata, message):
    try:
        controller.setFreq(message.payload)
    except ValueError:
        pass
...
if __name__ == "__main__":
    ...
    try:
        ...
        print (datetime.utcnow().strftime('[%Y-%m-%d %H:%M:%S.%f')[:-3]+"]\tController Firmware:"+str(ver))
    except:
        print (datetime.utcnow().strftime('[%Y-%m-%d %H:%M:%S.%f')[:-3]+"]\tError while querring firmware")
        closeAll()
        raise
        quit()
